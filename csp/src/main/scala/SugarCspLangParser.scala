 ...
 */
  ...
  def AllDifferentConstraint: Parser[SugarCspConstraint] =
    "(" ~> ("alldifferent" ~ rep(SugarTerm)) <~ ")" ^^ {case op ~ terms => SAlldifferent(terms)} |
    "(" ~> ("alldifferent" ~ "(" ~ rep(SugarTerm)) <~ "))"^^ {case op ~ terms => SAlldifferent(terms)}
}


object cspFactory {
  private[this] def numFactory(i: SNum): Num = Num(i.n)
  ...
  private[this] def termFactory(t: SugarCspTerm): Term = {
    t match {
      case i: SNum => numFactory(i)
      case x: SIntVar => varFactory(x)
      case SAdd(ts: Seq[SugarCspTerm]) => Add(ts.map(i => termFactory(i)))
  ...
  private[this] def constraintFactory(c: SugarCspConstraint): Constraint = {
    c match {
      ...
      case SAlldifferent(ts: Seq[SugarCspTerm]) => Alldifferent(ts.map(t => termFactory(t)))
    }
  }
